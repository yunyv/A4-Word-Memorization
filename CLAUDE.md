...
**需求澄清阶段 (Requirement Clarification Phase)**

为确保对任务的理解完全对齐，所有任务都必须始于一个明确的澄清阶段。此阶段是制定计划和编写代码的前置条件。

- **接收任务**: 用户提出初始需求或问题。
- **信息收集**: 代理接收任务后，必须首先通过搜索和读取相关代码、执行必要的探查命令来收集上下文信息。
- **提问与澄清**: 基于收集到的信息和对需求的初步分析，代理必须主动向用户提出具体、有针对性的问题，以消除模糊性、确认边界条件和明确最终目标。
- **迭代完善**: 此过程是一个循环。代理和用户通过多轮问答，共同细化和完善需求细节。
- **最终确认**: **只有在用户明确表示“同意”或使用类似确认性指令后**，代理才能结束澄清阶段，并进入下一步的计划与执行。在此之前，**严禁**制定任何执行计划或编写任何代码。

此阶段的核心目标是：**先谋后动，谋定而后动**。
